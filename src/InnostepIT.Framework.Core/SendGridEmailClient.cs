...
namespace InnostepIT.Framework.Core
{
    public class SendGridEmailClient : IEmailClient
    {
        ...
        public async Task SendAsync(string subject, string toSenderEmail, string toSenderName, string plainTextContent = "", string htmlContent = "")
        {
            ...
            {
                _logger.LogDebug($"{additionalHeaders.Length} additional email headers are configured and will be applied to the email.");
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                var errorMessage = $"{typeof(SendGridEmailClient)}: Failed to send email to {toSenderEmail} because of following errors: {errorMessages}";
